...
A later, separately approved feature-branch runtime checkpoint uses these six
approved victory assets only for mapped top-three characters. The runtime URLs
...
integration. The remaining six victory poses, twelve reactions, and backdrop
remain outside runtime use and under their existing visual-approval gates.
