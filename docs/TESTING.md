...
## Slice 5 Arc Blade — approved scope, no gameplay yet

`docs/SLICE-5-ARC-BLADE-SCOPE.md` specifies the approved automated/live matrix under amendment 2.15 / ADR-076. Manny approved the complete scope, including new range/speed/cadence values and the safe owner-return catch, and governance publication in Work on September 11, 2026. No Arc test pass or playable Arc test link is claimed. The primary future fixed-item route must preserve normal racing and unrestricted ITEM input. Per-leg hit deduplication must also prevent a turnaround from retriggering a continuous overlap. Validate actual controller/spin/camera integration, owner catch/cancellation, counters and shared capacity, not just trajectory-helper math. Run the normal full local gate and clean-install hosted CI before gameplay publication.
