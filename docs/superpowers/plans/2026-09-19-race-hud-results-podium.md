...
- [ ] Run focused suites for race HUD, mini-map, results/podium, app shell, route-night UI, item art, race results, and Character Select.
- [ ] Run the full repository sequence: `npm run typecheck`, `npm run lint`, `npm run test:ci`, `npm run build`, branding/runtime-asset validation, `git diff --check`, and `git lfs fsck`.
- [ ] Record the complete test-file/test-count and coverage output, the production-build result, runtime asset hashes, and any known nonblocking warnings without changing unrelated historical formatting.
...
- [ ] Run the hosted PR CI and post-merge CI/Pages workflow only through the normal repository process. Do not claim deployed acceptance from local or CI-only evidence.
