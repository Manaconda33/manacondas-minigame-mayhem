...
describe('Blaze deterministic acceptance fixtures', () => {
  it('accepts only the approved Blaze query combinations', () => {
    expect(prismaticTestFromSearch('?testItem=blaze-orbs&testBlaze=hit')).toEqual({
      mode: 'blaze-hit',
      expired: false,
    });
    expect(
    ...
    expect(prismaticTestFromSearch('?testItem=blaze-orbs&testBlaze=prismatic')).toBeNull();
  ...
  it.each([
    ...
    'spawns a production Blaze orb for the Prismatic %s encounter',
    (phase, initialRemaining) => {
      const track = new CircuitAlpha();
      const runtime = runtimeAt(track);
      const fixture = new PrismaticCounterFixture(
        prismaticTestFromSearch(
