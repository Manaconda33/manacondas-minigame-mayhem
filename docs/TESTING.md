...
## Vision-Obscuring Ink Splat governance test contract - local implementation in progress

Amendment 2.18 / ADR-079 and `docs/SLICE-5-INK-SPLAT-SCOPE.md` define the Ink evidence gate. Following governance publication PR #147 and Manny's separate implementation authorization, the local feature branch now contains the bounded Ink implementation and focused tests. This section does **not** claim hosted publication, deployment, or live acceptance.

Automated coverage must prove progress-authoritative all-racers-ahead targeting across lap/wrap cases; tied/behind/owner/finished/invalid exclusions; no-target inventory rollback; valid-use exact-once consumption even if all targets are immune; mixed immunity; 2.50-second refresh-without-stacking; Prismatic blocking without retroactive cleanse; pause/recovery/finish/restart/hub/disposal lifecycle; bounded/monotonic human overlay state below HUD/touch controls; deterministic smooth 0.55 m AI path-noise amplitude, 0.080-second decision latency, and 0.88 steering precision; legal-road bounding; unchanged AI speed/acceleration/rubber-band and race authority; zero shared physics-slot use; unchanged probability matrix; and a real `ItemEffectDispatcher` / `KartTimeTrial` / controller / AI / HUD-view integration path. Helper-only tests are insufficient. The current branch covers the focused state/audio and production runtime seams; local repository validation passes, while hosted publication and live gates remain open.

The primary route is `?testItem=ink-splat`. A deterministic incoming-Ink fixture exercises the human overlay without enabling general AI item tactics. Prismatic protected/expired fixtures verify actual resolved application/blocking. A miss, no-target setup, invalid/finished target, or fixture that never reaches the intended encounter is **INCONCLUSIVE**, not PASS. Desktop/mobile live acceptance must later cover effect readability, approximately 2.5-second fade, HUD/control usability, repeat refresh, AI navigability, pause/recovery/finish cleanup, Prismatic protected/expired behavior, and normal unforced-build isolation.
