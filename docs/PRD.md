...
## Approved implementation amendment 2.21 - Hyper-Drive Rocket governance scope

Approved September 17, 2026. PRD Section 15.16, ADR-082, and `docs/SLICE-5-HYPER-DRIVE-ROCKET-SCOPE.md` convert the existing Hyper-Drive Rocket requirements into a bounded operational contract: one atomic inventory charge; existing rank 6-8 and at-least-45-meter negative leader-gap eligibility; an exactly 6.0 race-second state; legal Circuit Alpha spline projection and autopilot through normal `KartController` input; a 1.25x speed-cap target with normal acceleration and surfaces; automatic overtakes earned through ordinary movement/checkpoint traversal; a final 0.30-second control return; source-scoped racer-contact and supported ground/area-hazard immunity; pause/lifecycle cleanup; and finite original procedural presentation. Governance publication completed through PR #154 at `ae977623bdc7a209634816e1cea8ef4a799b98c8`; post-merge validation and Pages run `35243454845` passed.

The approved scope forbids direct transform, velocity, checkpoint, lap, rank, finish, or racer-stat mutation; teleportation; forced first-place placement; uncontrolled forward physics; silent projectile immunity; selector/probability changes; and a second race-authority system. It keeps general AI item acquisition/use/tactics, broader projectile behavior, accepted item changes, dependencies, issue #106, and Slice 6 final polish outside this checkpoint. This scope is not gameplay, VFX, audio, or presentation implementation authorization; a separate implementation gate remains required.

## Approved implementation amendment 2.22 - Full AI item tactics

Approved September 17, 2026 after Hyper-Drive Rocket live acceptance. PRD Section 21.7, ADR-083, and `docs/SLICE-5-AI-ITEM-TACTICS-SCOPE.md` authorize the bounded full-AI item-use increment. All seven AI racers may acquire and use every eligible governed item through the existing one-slot inventory, selector, dispatcher, effect systems, and `KartController` boundaries. Existing item definitions, charges, cooldowns, probabilities, rank/gap values, accepted mechanics, and global object limits remain unchanged.

AI timing uses a stateless `AiItemPolicy`: first legal opportunity is the baseline, while target availability, rear pressure, defensive threats, shared capacity, and useful driving lines may hold an item. Kinetic/Seeker/Apex/Ink use progress-valid ahead targets; Blast/Slick and directional multi-charge items choose forward/backward intent from observed racers; Shockwave waits for a real incoming projectile, hostile hazard, or close racer; Nitro Surge/Overdrive prefer a straight or recovery line; Rocket activates promptly; and Prismatic protects a threatened or leading racer. Policy thresholds are implementation data in the AI scope document and are not item-balance changes.

AI effects are committed atomically through `ItemSystem` and `ItemEffectDispatcher`. All active `RacerEffects` drive modifiers reach AI `KartController` input, and Rocket composes through its existing legal autopilot seam. Steering, lane selection, hazard avoidance, rubber-banding, checkpoints, laps, finish ordering, and race authority remain in their current owners. No direct transform, velocity, progress, rank, finish, or racer-stat mutation is allowed. AI finish cleanup clears inventory and active item state without changing recovery semantics.

The opt-in deterministic fixture `?testAiItem=<item-id>&testAiRacer=ai-1` scopes the next collected item to one valid AI racer while preserving roulette and dispatch legality; `?testItem=<item-id>` remains player-only. Automated evidence covers all fifteen policy decisions, defensive/no-target waits, generic Overdrive/Rocket activation, controller integration, fixture isolation, unchanged probability evidence, item interactions, lifecycle, and existing AI regressions. Hosted publication, desktop/mobile review, soak/performance evidence, and final Slice 5 closure remain separate gates. AI-specific presentation polish and Slice 6 remain out of scope.
