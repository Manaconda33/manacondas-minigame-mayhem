...
status_start = status.index('## Current slice')
status_end = status.index('## Alex integration and deployment checkpoint')
status_current = '''## Current slice
...
'''
status_path.write_text(status[:status_start] + status_current + status[status_end:])

readme_path = Path('README.md')
readme = readme_path.read_text()
readme_start = readme.index('## Current state')
readme_end = readme.index('## Live test build')
readme_current = '''## Current state
...
'''
readme_path.write_text(readme[:readme_start] + readme_current + readme[readme_end:])

testing_path = Path('docs/TESTING.md')
testing = testing_path.read_text()
heading = '## Slice 5 roulette, HUD, and input checkpoint'
if heading in testing:
    raise SystemExit('Slice 5 HUD/input testing section already exists')
marker = '## Slice 4 AI/grid manual matrix'
if testing.count(marker) != 1:
    raise SystemExit(f'Expected one Slice 4 marker, found {testing.count(marker)}')
testing_section = '''## Slice 5 roulette, HUD, and input checkpoint
...
'''
testing_path.write_text(testing.replace(marker, testing_section + marker, 1))
