...
## Vision-Obscuring Ink Splat governance test contract - AI tuning amendment in progress

Amendments 2.18-2.19 / ADR-079-080 and `docs/SLICE-5-INK-SPLAT-SCOPE.md` define the Ink evidence gate. Gameplay and presentation implementation merged through PR #148 at `2df6bf372b01e8a0f13c4bad71737ef8f5ab415d`; post-merge validation/Pages run `35168880807` passed. Manny's live review found the original AI impairment below the intended perceptual threshold, and the approved tuning update remains on a separate follow-up branch. This section does **not** claim tuned hosted publication, deployment, or live acceptance.

Automated coverage must prove progress-authoritative all-racers-ahead targeting across lap/wrap cases; tied/behind/owner/finished/invalid exclusions; no-target inventory rollback; valid-use exact-once consumption even if all targets are immune; mixed immunity; 2.50-second refresh-without-stacking; Prismatic blocking without retroactive cleanse; pause/recovery/finish/restart/hub/disposal lifecycle; bounded/monotonic human overlay state below HUD/touch controls; deterministic smooth 0.95 m AI path-noise amplitude, 0.160-second decision latency, and 0.74 steering precision; legal-road bounding; unchanged AI speed/acceleration/rubber-band and race authority; zero shared physics-slot use; unchanged probability matrix; and a real `ItemEffectDispatcher` / `KartTimeTrial` / controller / AI / HUD-view integration path. Helper-only tests are insufficient. The deployed baseline covers the focused state/audio and production runtime seams; tuned hosted publication and live gates remain open.
