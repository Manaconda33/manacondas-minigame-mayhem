...
- **Date:** 2026-09-17.
- **Status:** **APPROVED TUNING AMENDMENT; implementation in progress.** The merged/deployed 2.18 implementation remains unchanged until the tuned follow-up is published.
...
- **Context:** The 2.18 implementation correctly delivered deterministic lateral noise, buffered steering decisions, and reduced steering precision, but the approved values were below the intended perceptual threshold during live review. The correction must improve visible imprecision without changing Ink targeting, duration, inventory, immunity, human coverage, speed authority, race authority, or lifecycle boundaries.
...
- **Rationale:** The original values were technically active but not perceptually legible in ordinary racing. The revised values are intended to produce visibly late, less precise corrections while retaining continuous legal navigation and avoiding a hidden speed penalty.
- **Boundary:** This amendment changes no item probability, racer statistic, track/checkpoint authority, AI item acquisition/use, accepted item behavior, dependency, binary asset, Overdrive, Rocket, or Slice 6 scope. Tuned gameplay publication, post-merge validation/Pages, and Manny's live acceptance remain separate gates.
