...
## Slice 5 Arc Blade — locally validated, publication and live acceptance pending

`docs/SLICE-5-ARC-BLADE-SCOPE.md` and amendment 2.15 / ADR-076 govern this implementation. Governance PR #138 merged at `7ce6511bc040d2b176ed528b687ed589fafd045d`; CI/Pages `35027045477` passed. Manny subsequently authorized gameplay with “Proceed” and continued it on September 16, 2026. Full local `npm run validate` passed **48 files / 431 tests**, **82.50% statement coverage**, strict typecheck, zero-warning lint, asset verification and production build. `git diff --check` and `git lfs fsck` passed. Hosted clean-install CI and approved gameplay publication remain required; these are local results, not deployed acceptance.
...
| Stress                           | `tests/arc-soak.test.ts`: 800 throws with 40 simultaneous objects and eight racer snapshots, finite transforms and bounded capacity/resources. An isolated non-coverage CPU observation recorded median 0.772 ms, p95 1.834 ms and maximum 7.863 ms across 1,386 full-capacity updates. This is not rendered/device performance certification or a pass for the final all-item budget.                                          |

The connected browser rejected the local preview with `ERR_BLOCKED_BY_CLIENT`. Rendered appearance, audible sound, device performance and Manny's live matrix remain unverified. Automated frustum and lifecycle checks do not replace those checks.

### Arc review instructions — use only after approved gameplay deployment

The primary route will be `?testItem=arc-blade`. Race normally, collect a box and let roulette finish, then use E, Left Shift or mobile ITEM whenever eligible. No stopping, target setup or diagnostic countdown is required. Check three forward throws, the 0.55-second cadence and charge count, reverse input still throwing forward, the curved outbound/return flight and safe owner catch without refund. Check ordinary rival spin and a legitimate separated return hit without repeated continuous-overlap damage.

Optional counter routes keep ordinary driving and ITEM use available:

| Query                                                                                         | Setup and verified outcome                                                                                                                                                                                                      |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `?testItem=shockwave&testArcBladeCounter=shockwave`                                           | Collect Shockwave. The badge announces an incoming Arc after a short heads-up and waits for a clear approach when needed. Activate when the blade enters 5 m; PASS requires the actual Shockwave clear before movement/contact. |
| `?testItem=prismatic-invincibility&testArcBladeCounter=prismatic&testArcBladePhase=protected` | Collect and activate Prismatic while racing. An incoming Arc is staged during protection; PASS requires confirmed absorption with unchanged velocity and spin.                                                                  |
| `?testItem=prismatic-invincibility&testArcBladeCounter=prismatic&testArcBladePhase=expired`   | Collect and activate Prismatic while racing. The diagnostic waits for actual expiry before staging the blade; PASS requires a verified contact applying the ordinary 0.85-second Arc spin.                                      |

Misses, interception, wall/catch/expiry, recovery or unsuitable geometry are INCONCLUSIVE, never a protection pass. Restart to retry. The optional diagnostic countdown only announces staging; it never blocks ITEM. Record the deployed commit, device/browser, badge and visible outcome for each case. A counter badge certifies only its measured encounter.

Complete the scope's remaining live checks: chase/rear and desktop/mobile readability and controls, original visuals/audio and volume, pause/recovery/finish/restart/hub cleanup, and accepted-item/AI hazard-response regression on the normal URL without test parameters. Do not distribute these query routes as playable Arc links until gameplay deployment is verified.
