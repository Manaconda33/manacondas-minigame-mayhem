...
**Slice 6 - UI/HUD Polish, Audio, Post Processing & Optimization - CIRCUIT ALPHA PBR PASS LIVE ACCEPTED; ROUTE NIGHT TITLE/HUB/UTILITY MERGED; CHARACTER SELECT IN FEATURE-BRANCH REVIEW**
...
Latest verified repository head on `main`: **`f86f5567855c064b8648414a784ec712c0086937`** (Route Night title/hub/controls/settings through PR #184; post-merge validation/Pages run `35400823101` passed).

Latest verified Slice 6 implementation checkpoint on `main`: **`f86f5567855c064b8648414a784ec712c0086937`** (Route Night title/hub/controls/settings through PR #184; hosted PR CI and post-merge validation/Pages run `35400823101` passed; the accepted Circuit Alpha PBR baseline remains governed by ADR-088).
...
**Slice 6:** **ACTIVE - THIRD BOUNDED INCREMENT IN FEATURE-BRANCH REVIEW.** Manny approved beginning Slice 6 on 2026-09-18 and authorized the material-coordinate / first Circuit Alpha PBR increment plus normal PR/CI/deployment publication. PR #178 is merged/deployed, and the Route Night title/hub/Controls/Settings increment is merged/deployed through PR #184. ADR-088 and `docs/SLICE-6-CIRCUIT-ALPHA-PBR-PASS-2026-09-18.md` govern the protected material baseline; ADR-090 and `docs/SLICE-6-CHARACTER-SELECT-CHECKPOINT-2026-09-19.md` govern the current Character Select branch checkpoint.

Current findings: deployed `main` now gives procedural loop/segment strips deterministic 2 m world-meter UVs and applies one provenance-tracked 1K Poly Haven Asphalt Track PBR set to the road/racing-wear surfaces only. Approved kart builders remain untouched. Settings persistence, graphics presets, and the mixer foundation are already published. The Route Night title/hub/Controls/Settings system is merged and deployed; this branch extends the same language into Character Select with the PRD's rotating 3D kart preview. The renderer still has no post-processing stack, there is no production engine/final-lap music system, the runtime pause lacks its final menu, and Results lacks the full Race Again / Change Driver / Hub flow.
...
**Route Night Character Select scope:** **APPROVED FOR IMPLEMENTATION 2026-09-19.** ADR-090 authorizes the bounded responsive Character Select redesign, real approved driver/kart asset binding, and isolated rotating kart preview. Publication and Manny's deployed visual-acceptance gate remain pending; no later Slice 6 screen is authorized.
...
**Next recommended action:** publish the bounded Route Night Character Select feature branch through the normal approval-governed PR flow, then stop at Manny's deployed Character Select visual-acceptance gate before beginning the race HUD, pause/results, final audio, post-processing, or later Slice 6 work.
...
## Slice 6 Route Night Character Select checkpoint - feature branch
...
The implementation replaces the earlier rounded Character Select scaffold with a clipped, route-dense driver checkpoint. It renders all twelve approved manifest entries with real portrait assets and fallback behavior, binds the selected production front driver art and fixed six-stat presentation, and preserves the existing race handoff and hub navigation. `src/ui/characterKartPreview.ts` loads the selected approved kart GLB through `GLTFLoader`, applies the manifest-governed visual yaw, rotates only when motion is allowed, and falls back to a procedural preview when WebGL or a kart asset is unavailable. Generated bay, route-energy, and hero-aura layers are atmospheric only; layout, copy, interaction, and identity remain live DOM/CSS/SVG or approved manifest assets.

Local evidence on this branch: **67 test files / 530 tests passed**, **80.57% statement / 75.98% branch / 86.03% function / 82.21% line coverage**, strict typecheck passed, zero-warning lint passed, branding/runtime-asset validation passed, production build passed, `git diff --check` passed, `git lfs fsck` passed, and targeted formatting checks for changed files passed. The repository-wide Prettier check still reports pre-existing formatting drift in unrelated historical files; no repository-wide formatting rewrite was applied. The cloud-browser attempt was blocked by `ERR_BLOCKED_BY_CLIENT` for the local preview URL, so no deployed browser-rendered acceptance is claimed. Publication, hosted deployment, and Manny's desktop/mobile Character Select visual-acceptance gate remain pending.
